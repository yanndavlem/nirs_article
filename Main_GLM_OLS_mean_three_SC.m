...
for i = 1:length(folders)
    raw(i,1) =  nirs.io.loadNIRx(['H:/Autres ordinateurs/Mon ordinateur/Yann/Data Analyse/1. Analyse NIRS Toolbox/Analyse_TPP/Analyse CRNL/fichiers_nirx/' folders(i).name],true); % importation, mettre dans les trois petits points le chemin vers le dossier contenat les donnÃ©es brutes
end
...
bad_chan = bad_channels_indices(raw,0.7);
...
% GLM
job = [];
job = nirs.modules.GLM_mean_three_sc;
job.type = 'OLS'; % Choisi le type de GLM
job.AddShortSepRegressors_mean_three_sc = 1; % Décide de prendre ou non le signal des shorts channels (0 : non / 1 : oui)
...
% dm2 = nirs.util.getdesign_matrix(sub_stats(1));
% imagesc(dm2);
% % save
...
a = sub_stats(i).table;
writetable(a,['H:/Autres ordinateurs/Mon ordinateur/Yann/Data Analyse/1. Analyse NIRS Toolbox/Analyse_TPP/Analyse CRNL/1. Script pour figure papier/V vs A R Analyses/OLS_trial_block_mean_three_sc/suj_' num2str(i) '_mean_three_sc.csv'],'Delimiter',',')
end
