...
for i = 1:size(tbl,1) %Additionne et verifie le nombre de trigger
    ...
    cell_trig(i) = {[num_trig_1 num_trig_2 num_trig_3]};
...
for j = find(trig_ok)
    ...
    %Ajoute une ligne spécial pour le 7e sujet
    if j == 7
...
% Stocke pour plus tard la table de triggers
...
%% Données avec shorts channels (LOGIQUE MODIFIÉE AVEC 3 PAIRES DE SC)

lc_idx = find(Hb(1).probe.link.ShortSeperation == 0); % recupere les numeros de canaux des long channels
% Définit les 6 indices SC cibles (Paire 1: 5,6; Paire 2: 25,26; Paire 3: 43,44)
...
for nxn = 1:length(lc_idx)
    ...
    % ====================================================================
    % DÉBUT DE LA LOGIQUE : TROUVER LE MEILLEUR SC (PARMI LES 6 CIBLES)
    % ====================================================================
    ...
    all_corrs_this_lc = zeros(length(Hb_nostim), length(target_sc_indices));
    ...
    for i = 1:length(Hb_nostim) % Boucle sur chaque sujet
        ...
        if max(target_sc_indices) > size(Hb_nostim(i).data, 2)
             error('Indice SC cible (%d) hors limites pour Hb.data (taille: %d)', max(target_sc_indices), size(Hb_nostim(i).data, 2));
        end
    ...
    % Calcule la moyenne des corrélations *absolues* (vecteur 1x6)
    avg_abs_corrs = mean(abs(all_corrs_this_lc), 1);
    
    % Trouve l'index *local* (1-6) du SC avec la plus haute corrélation
    [~, best_sc_local_idx] = max(avg_abs_corrs);
    
    % Récupère l'index *global* de ce SC (ex: 25)
    best_sc_global_idx = target_sc_indices(best_sc_local_idx);
    ...
    if mod(best_sc_global_idx, 2) == 1 % Si l'indice est impair (ex: 5, 25, ...)
        pair_global_idx = best_sc_global_idx + 1; % Sa paire est l'indice pair suivant
    else % Si l'indice est pair (ex: 6, 26, ...)
    ...
    % fprintf('  -> LC %d: Best correlated global SC index %d (from 6). Using pair [%d, %d].\n', ...
    ...
    for i = 1:size(sub_stats,2)
        % extracts probe object
        probe_tbl = data_cor(i).probe.link;
        
        % extracts short-channel data (time*2)
        data_sc_PCA = data_cor(i).data(:,good_SC);
        % extracts all beta values
        a = sub_stats(i).table;
        ...
        % creates a matrix of beta values
...
%Export les données avec short channel
% J'ai changé le nom du fichier pour refléter la nouvelle analyse
writetable(r_table_average_with_sc,'H:/Autres ordinateurs/Mon ordinateur/Yann/Data Analyse/1. Analyse NIRS Toolbox/Analyse_TPP/Analyse CRNL/1. Script pour figure papier/V vs A R Analyses/Average_trial_block_corr_three_sc/r_table_average_corr_three_sc.csv','Delimiter',',')