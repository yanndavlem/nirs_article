classdef AddShortSeperationRegressors_corr_three_SC < nirs.modules.AbstractModule
    ...
    methods
        ...
        function data = runThis( obj, data )
            for i = 1:numel(data)
                ...
                dd=data(i).data(:,obj.whichSC);
                %Pour le three_sc mettre obj.whichSC à la place de lstss
                
                %dd=dd-ones(size(dd,1),1)*mean(dd,1); # no normalization of
                %SC data
...
%                 dd=orth(dd);
                for j=1:size(dd,2)
                    ...
                    st.time=data(i).time;
                    st.vector=dd(:,j);
                    st.vector=st.vector-mean(st.vector); %à commenter pour verif
                    st.vector=st.vector./sqrt(var(st.vector));%à commenter pour verif
