...
%% 3. Analysis: GLM with Mean of Three Specific SCs
% Logic: Use a custom GLM module that uses the mean of 3 specific SCs.


% NOTE: 'nirs.modules.GLM_mean_three_sc' is a CUSTOM module available on our GITHUB. 
% It must be present in the toolbox path nirs-toolbox-master/nirs/modules/GLM_mean_three_sc.m
job = [];
if exist('nirs.modules.GLM_mean_three_sc', 'class')
    job = nirs.modules.GLM_mean_three_sc();
else
    error('Custom module nirs.modules.GLM_mean_three_sc not found.');
end

job.type = 'OLS';
job.AddShortSepRegressors_mean_three_sc = 1; % Activate specific logic

sub_stats = job.run(Hb);

% Visualization of Design Matrix (First subject)
dm = nirs.util.getdesign_matrix(Hb(1));
figure; imagesc(dm); title('Design Matrix (Mean Three SC)'); colorbar;
...
save(fullfile(output_dir, 'stats_trial_block_OLS_MeanThreeSC.mat'), 'sub_stats');
...
for i = 1:length(sub_stats)
    ...
    % Construct filename
    filename = sprintf('suj_%d_mean_three_sc.csv', i);
