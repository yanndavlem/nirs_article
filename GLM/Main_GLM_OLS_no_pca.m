...
%% 3. Analysis: GLM with All Short Channels (No PCA)
% Logic: Use a custom GLM module that includes all SCs as individual regressors.

% NOTE: 'nirs.modules.GLM_no_pca' is a CUSTOM module available on our GITHUB. 
% It must be present in the toolbox path nirs-toolbox-master/nirs/modules/GLM_no_pca.m
job = [];
if exist('nirs.modules.GLM_no_pca', 'class')
    job = nirs.modules.GLM_no_pca();
else
    error('Custom module nirs.modules.GLM_no_pca not found.');
end

job.type = 'OLS';
job.AddShortSepRegressors_no_pca = 1; % Activate specific logic

sub_stats = job.run(Hb);

% Visualization of Design Matrix (First subject)
dm = nirs.util.getdesign_matrix(Hb(1));
figure; imagesc(dm); title('Design Matrix (No PCA)'); colorbar;
...
save(fullfile(output_dir, 'stats_trial_block_OLS_NoPCA.mat'), 'sub_stats');

% Export Individual Subject Stats to CSV
for i = 1:length(sub_stats)
    stats_table = sub_stats(i).table;
    
    % Construct filename: suj_1_all_sc_no_PCA_V2.csv, etc.
    filename = sprintf('suj_%d_all_sc_no_PCA_V2.csv', i);
