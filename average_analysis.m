...
% ajoute au path MATLAB la NIRSToolbox (mettre l'acc�s � la place des trois petits
% points
...
for i = 1:length(folders)
    raw(i,1) =  nirs.io.loadNIRx(['H:/Autres ordinateurs/Mon ordinateur/Yann/Data Analyse/1. Analyse NIRS Toolbox/Analyse_TPP/Analyse CRNL/fichiers_nirx/' folders(i).name],true); % importation, mettre dans les trois petits points le chemin vers le dossier contenat les donn�es brutes
end
...
Hb_nostim_sc = add_SC_regressors_PCA(Hb_nostim);
...
table_test = r_table_average_with_sc(r_table_average_with_sc.type == "hbo" & r_table_average_with_sc.ID == "sub_1" & r_table_average_with_sc.channel == "1-1" & r_table_average_with_sc.stim == "trig1",:);
plot(table_test.data)
plots_average(average_screg,'hbo',[-20 20],"HbO average SC correction")

%% Export fichier CSV

%Export les données avec short channel dans un tableau avec le en csv
writetable(r_table_average_with_sc,'H:/Autres ordinateurs/Mon ordinateur/Yann/Data Analyse/1. Analyse NIRS Toolbox/Analyse_TPP/Analyse CRNL/Script_R/Resultat_GLM/Average_trial_block_with_sc/r_table_average_with_sc.csv','Delimiter',',')
